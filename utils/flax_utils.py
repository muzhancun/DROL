...
class TrainState(flax.struct.PyTreeNode):
    ...
    def __call__(self, *args, params=None, method=None, **kwargs):
        ...
        if params is None:
            params = self.params
        variables = {'params': params, **self.model_state}
    ...
    def apply_loss_fn_with_state(self, loss_fn):
        """Apply the loss function with mutable model state and return updated state.

        `loss_fn` should have signature `(params, model_state) -> (loss, (info, new_model_state))`.
        """

        (loss, (info, new_model_state)), grads = jax.value_and_grad(loss_fn, argnums=0, has_aux=True)(
        ...
        return self.apply_gradients(grads=clipped_grads, model_state=new_model_state), info
